...
def bin2hex(s):
    return hex(int(s, 2))[2:].zfill((len(s) + 3) // 4)
...
def s_box_substitution(bits):
    ...
    for i in range(8):
        ...
        col = int(block[1:5], 2)
        output += bin(S_BOX[i][row][col])[2:].zfill(4)
    return output
...
def des_round(left, right, subkey):
    ...
    xored_right = xor(expanded_right, subkey)
    substituted_right = s_box_substitution(xored_right)
    permuted_right = permute(substituted_right, P)
    new_right = xor(left, permuted_right)
    return right, new_right


# Function to perform DES encryption/decryption
def des(key, data, decrypt=False):
...
if __name__ == "__main__":
    ...
    # key = "0f1571c947d9e859"  # 64-bit key
    # plaintext = "02468aceeca86420"  # 64-bit plaintext

    print("Key:", key)
    print("Plaintext:", plaintext)

    # Encrypt
    ciphertext = des(key, plaintext)
    print("Ciphertext:", ciphertext)

    # Decrypt
    decrypted_text = des(key, ciphertext, decrypt=True)
